...
BEGIN
  ...
  FOR it IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    INSERT INTO public.order_items(order_id,size_id,qty,unit_price,line_total,honey_kg_used,cogs_line)
     VALUES (new_order_id,size_rec.id,qty,unit_price,line_total,honey_kg,v_cogs_line);
  END LOOP;

  IF v_total_honey > db.kg_remaining THEN
    RAISE EXCEPTION 'Stok madu di dandang tidak cukup (sisa % kg, butuh % kg)', db.kg_remaining, v_total_honey;
  END IF;
  UPDATE public.dandang_balance SET kg_remaining = kg_remaining - v_total_honey, updated_at=now() WHERE id=1;
