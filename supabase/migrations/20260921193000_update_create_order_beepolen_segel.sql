-- Ensure create_order explicitly excludes BEEPOLEN and sizes 30, 100, 130 from segel (jar seals)
...
BEGIN
  ...
  FOR it IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    IF honey_kg > db.kg_remaining THEN RAISE EXCEPTION 'Stok % di dandang tidak cukup (sisa % kg, butuh % kg)', htype, db.kg_remaining, honey_kg; END IF;
    UPDATE public.dandang_balance SET kg_remaining = kg_remaining - honey_kg, updated_at=now() WHERE honey_type=htype;

    -- Resolve segel item ID: BEEPOLEN and sizes 30g, 100g, 130g do NOT use honey jar seals
    v_segel_item_id := NULL;
    IF htype != 'BEEPOLEN' AND size_rec.weight_grams NOT IN (30, 100, 130) THEN
