-- Update create_order function to map 130 gr bottles to 100 gr bottles
...
-- Update import_historical_order function to map 130 gr bottles to 100 gr bottles
...
-- Update process_order_return function to map 130 gr bottles to 100 gr bottles
...
BEGIN
  ...
  FOR item IN SELECT * FROM public.order_items WHERE order_id = _order_id LOOP
    ...
    IF item_cond = 'aman' THEN
      UPDATE public.packaging_items SET current_stock = current_stock + item.qty WHERE type='botol' AND size_id = v_bottle_size_id;
      UPDATE public.packaging_items SET current_stock = current_stock + item.qty WHERE type='stiker' AND size_id = item.size_id;
      
      IF sg_item IS NOT NULL AND size_rec.weight_grams NOT IN (100, 130) THEN
        UPDATE public.packaging_items SET current_stock = current_stock + item.qty WHERE id = sg_item;
      END IF;
    ELSE
      -- If condition is 'rusak', record cost as packaging loss
      v_pkg_loss := v_pkg_loss + item.qty * COALESCE((SELECT avg_cost FROM public.packaging_items WHERE type='botol' AND size_id = v_bottle_size_id), 0)
                               + item.qty * COALESCE((SELECT avg_cost FROM public.packaging_items WHERE type='stiker' AND size_id = item.size_id), 0);
      
      IF sg_item IS NOT NULL AND size_rec.weight_grams NOT IN (100, 130) THEN
        v_pkg_loss := v_pkg_loss + item.qty * COALESCE((SELECT avg_cost FROM public.packaging_items WHERE id = sg_item), 0);
      END IF;
    END IF;
...
-- Update delete_order_return function to map 130 gr bottles to 100 gr bottles
