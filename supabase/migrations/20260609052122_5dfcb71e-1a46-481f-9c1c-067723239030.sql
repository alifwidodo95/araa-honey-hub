...
BEGIN
  ...
  SELECT * INTO d FROM public.dandang_balance LIMIT 1;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Saldo dandang tidak ditemukan';
  END IF;

  IF d.kg_remaining < t.kg_added THEN
    RAISE EXCEPTION 'Saldo madu di dandang (%) lebih kecil dari kg yang akan dihapus (%). Madu sudah terpakai.', d.kg_remaining, t.kg_added;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.delete_dandang_transfer(uuid) TO authenticated;
